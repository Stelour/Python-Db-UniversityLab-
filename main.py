...
def change_in_db(new_name, directory, bio, old_name):
    with open(directory, 'rb') as photo:
        ...
        with sqlite3.connect('AmDB.db') as con: con.execute("""UPDATE people SET name = ?, photo = ?, bio = ? WHERE name = ?""", data)
def input_in_db(name, directory, bio):
    with open(directory, 'rb') as photo:
        image = photo.read()
        data = (name, image, bio)
        with sqlite3.connect('AmDB.db') as con: con.execute("""INSERT INTO people (name, photo, bio) VALUES (?, ?, ?)""", data)
def get_db():
    list_name, list_photo, list_bio = [], [], []
    with sqlite3.connect('AmDB.db') as con: 
        data = con.execute("SELECT name FROM people")
        for row in data: list_name.append(row)
        data = con.execute("SELECT photo FROM people")
        for row in data:photo = PhotoImage(data=row[0]); list_photo.append(photo)
        data = con.execute("SELECT bio FROM people")
        for row in data: list_bio.append(row)
...
def bar():
    ...
    Label(text = "      F1 - Справка      |     F2 - Добавить      |     F3 - Удалить      |     F4 - Изменить      |     Ctrl + X - Выход", bd=1, relief=SUNKEN, anchor=W, bg="#9797FF").pack(side = BOTTOM, fill = X)
...
def edit(event = NONE):
    ...
    if not(listbox.curselection()): messagebox.showinfo("Уведомление", "Выберите элемент"); return
    else: index = listbox.curselection()[0]
    ...
    def confirm():
        for i in range(listbox.size()):
            if entryName.get() == listbox.get(i): messagebox.showwarning("Предупреждение", "Такое имя уже есть"); return
        if len(entryName.get()) == 0 or len(textBio.get("1.0", "end-1c")) == 0 or len(fileEntry.get()) == 0: messagebox.showwarning("Предупреждение", "Отсутствует имя, описание или путь до фотографии"); return
        else:
...
def output_again(event):
    list_name, list_photo, list_bio = get_db()
    w = event.widget
    if w.curselection(): index = int(w.curselection()[0])
    else: return
...
def output_name():
    ...
    for i in list_name: listbox.insert(END, f"{i}")
bar()
output_name()
